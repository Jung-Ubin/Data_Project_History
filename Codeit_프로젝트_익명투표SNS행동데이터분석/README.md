...
- 동일 이벤트 연속 발생 압축  
- 90분 이상 간격 시 세션 재분리  
  (다음 로그까지 90분 이상 간격 발생 시 앱 종료 후 시작으로 간주)    
- 유저별 event_flow 생성    
...
- 1,100만 건 이상의 대용량 로그 처리로 인해 성능 개선이 필요한 상황
...
이 프로젝트를 통해  
행동 로그 분석 → 문제 구조화 → UX·비즈니스 개선 까지의 과정을 경험함  

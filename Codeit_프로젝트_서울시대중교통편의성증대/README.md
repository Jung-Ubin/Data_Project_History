...
```python
# 파생 변수 생성
df["인구밀집도대비_정류장수"] = df["정류장수"] / df["인구밀집도"]
df["인구밀집도대비_노선수"] = df["노선수"] / df["인구밀집도"]
```
...
- 취약 지역 후보 지도 기반 탐색
```python
# 인구 밀집도 대비 정류장·노선 시각화
ax[0].barh(df_stop_sorted["area"], df_stop_sorted["bus_stop"])
ax[1].barh(df_line_sorted["area"], df_line_sorted["route"])
```
